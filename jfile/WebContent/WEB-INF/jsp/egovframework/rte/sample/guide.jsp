...
<!-- 		<li><a href="#talk">Egov 에 바라는 점</a></li> -->
    ...
    <div id="jfileIntro">
		...
		<ul>
			...
			<li><a>2) 서버에 파일이 저장되는 root 폴더의 위치를 업무마다 다르게 저장하고 해야함. 예) 신한은행 : /app/s001, 국민은행 : /app/k001, 우리은행 : /app/w001.</a></li>
    ...
    <div id="gettingStarted">
		...
		<p>
			- jfile 업/다운로드 컴포넌트는 현재 oracle 데이타베이스 환경에 맞게 구축되어 있습니다. 추후 DBMS 벤더와 무관하게 동작가능 하도록 업그레이드 될 것입니다.
		...
		<ul>
			<li><a>1) jfile.zip 파일을 임의의 디렉토리에 압축을 풀고 Eclipse > PackageExplorer > Import > General > Existing Projects into Workspace 를 선택후
			해당 파일을 선택한 후 ok 버튼을 클릭합니다. </a>			
			</li>
			</br>
			<li><a>2) 테이블 생성을 합니다. 테이블 생성 스크립트 위치 : ([프로젝트루트폴더]/docs/J_ATTACHFILE.sql) </a>
				<fieldset>
					<legend><b>Table Script</b></legend>
					
					CREATE TABLE J_ATTACHFILE <br/>
					(<br/>
					  FILE_ID               VARCHAR2(13 BYTE)       NOT NULL,<br/>
					  FILE_SEQ              INTEGER                 NOT NULL,<br/>
					  FILE_NAME             VARCHAR2(100 BYTE)      NOT NULL,<br/>
					  FILE_SIZE             INTEGER,<br/>
					  FILE_MASK             VARCHAR2(100 BYTE),<br/>
					  DOWNLOAD_COUNT        INTEGER,<br/>
					  DOWNLOAD_EXPIRE_DATE  VARCHAR2(8 BYTE),<br/>
					  DOWNLOAD_LIMIT_COUNT  INTEGER,<br/>
					  REG_DATE              DATE                    DEFAULT SYSDATE,<br/>
					  DELETE_YN             VARCHAR2(1 BYTE)<br/>
					);<br/>
					<br/>
					
					CREATE UNIQUE INDEX J_ATTACHFILE_PK ON J_ATTACHFILE<br/>
					(FILE_ID, FILE_SEQ);<br/>
					<br/>
					
					ALTER TABLE J_ATTACHFILE ADD (<br/>
					  CONSTRAINT J_ATTACHFILE_PK<br/>
					  PRIMARY KEY<br/>
					  (FILE_ID, FILE_SEQ)<br/>
					  USING INDEX J_ATTACHFILE_PK);<br/>       
				</fieldset>
			</li>
			<li><a>3) 데이타베이스 접속 설정 정보를 자신의 환경에 맞게 변경합니다. 현재는 oracle 만 가능함. <br/>
			          설정 파일 위치 : ([프로젝트루트폴더]/src/main/resources/egovframework/spring/context-datasource.xml) <br/>
			    <fieldset>
					<legend><b>context-datasource.xml</b></legend>
					&lt;!-- oracle --&gt;<br/>
				    &lt;bean id="dataSource" class="org.apache.commons.dbcp.BasicDataSource" destroy-method="close"&gt;<br/>
				        ...
				        &lt;property name="url" value="jdbc:oracle:thin:@[hostname]:[port]:[sid]"/&gt;<br/>
				        &lt;property name="username" value="[userid]"/&gt;<br/>
			...
			<li><a>4) Java EE Perspective > New > Server > Apache > Tomcat 6.0 Server > jfile 선택 후 finish 버튼을 클릭합니다.			
			</a></li>
			<br/>
			
			<li><a>5) 웹 브라우저에서 http://127.0.0.1:8080/jfile/ 에 접속합니다.			
			</a></li>
		</ul>
	...
	<div id="options">
		...
		<ul>
			<li><a>Jsp 페이지 상단에 &lt;%@ taglib uri="http://www.ibizsoftware.com/tags/jsp" prefix="jwork"%&gt; 태그를 추가합니다.
			<fieldset>
			...
			<li><a>④ beanId="jfileService" 는 [프로젝트루트폴더]/src/main/resources/egovframework/spring/context-jfile.xml 등록 되어 있는 beanId 입니다.</a></li>
	...
	<div id="configuration">
		...
		<ul>
			...
			<fieldset>
			<legend><b>[프로젝트루트폴더]/src/main/resources/egovframework/spring/context-jfile.xml</b></legend>
				
				① &lt;aop:aspectj-autoproxy proxy-target-class="false" /	 &gt;<br/>
				② &lt;bean class="egovframework.com.jfile.aspect.MultipartResolverDisablingAspect" /&gt;<br/>
				③ &lt;bean id="multipartResolver" class="org.springframework.web.multipart.commons.CommonsMultipartResolver"/&gt;<br/>				
				④ &lt;bean id="download" class="egovframework.com.jfile.view.DownloadView" /&gt;<br/>
				⑤ &lt;bean id="jsonView" class="egovframework.com.jfile.view.JSonView" /&gt;<br/>
				⑥ &lt;bean id="cipherService" class="egovframework.com.jfile.security.service.CipherServiceImpl" &gt;<br/>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;property name="jcrypto" &gt;<br/>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;bean class="egovframework.com.jfile.security.GenericJCrypto"/ &gt;<br/>
				 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/property &gt;<br/>
				 &nbsp;&nbsp;&nbsp;&nbsp;&lt;/bean&gt;<br/>
			</fieldset>
			<br/>
    ...
    <div id="simple">
		...
		<legend><b>사용자 정의 Jsp 파일</b></legend>
			...
			&lt;link href="&lt;c:url value="/resources/swfupload/css/custom.css" /&gt;" rel="stylesheet" type="text/css" /&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/jquery/1.3.2/jquery-1.3.2.min.js" /&gt;"&gt;&lt;/script&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/json.js" /&gt;"&gt;&lt;/script&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/jwork/jwork.fileUpload.js" /&gt;"&gt;&lt;/script&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/jwork/jwork.fileDownload.js" /&gt;"&gt;&lt;/script&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/swfupload/swfupload/swfupload.js" /&gt;"&gt;&lt;/script&gt;<br/>
			&lt;script type="text/javascript" src="&lt;c:url value="/resources/swfupload/js/swfupload.queue.js" /&gt;"&gt;&lt;/script&gt;<br/><br/>
			</fieldset>
			<fieldset>
	...
	<div id="talk">
		...
		<h2>Egov 로 공모전을 준비하면서...</h2>
		<p>
			Egov 는 Spring 기반의 프레임워크를 사용하고 오픈소스 정책을 펼침으로서 Java 오픈소스에 관심있는 개발자들의 참여를 유도해서 그 품질을 향상시켜 나가려 하는것 같습니다.
			Egov 기반으로 공모전을 준비하면서 Egov 가 자랑하는 다양한 종류의 Plug-in 들과 서비스 모듈들에 깊은 감명을 받았습니다. 다만 몇 가지 아쉬운 점 때문에 조금 고생(^^;)을 하면서
			이 점은 개선해 주셨으면 하는 바람이 있습니다.
		</p>
		<br/>
		<ul>
   			<li><a>
   				Spring 에서 제공하는 HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest(); 를
   				사용 할 수 가 없습니다.
   				<br/>
   				저 기능을 활용하면 굳이 request 객체를 Controller 단에서 부터 끌고 다니지 않아도 어느 곳에서나 현재 쓰레드에 존재하는 request 객체를 얻어 올수 있는데 저걸 썻더니
   				egovframework.rte.ptl.mvc.filter.HTMLTagFilterRequestWrapper 클래스에서 getParameterValues 하는 부분에서 오작동을 하는 거 같습니다.
   				특정 순간에 request 에 세팅했던 parameter 값들이 사라져 버립니다.
   			</a></li>
   			
   			</br>
   			<li><a>
   				properties 파일에서 key 와 value 를 읽어오는 클래스를 활용하려면 사용하고자 하는 클래스가 Spring bean 으로 등록되지 않으면 사용 할 수 없습니다.
   				bean 으로 등록되지 않은 Pojo 클래스에서도 Util 형태로 쉽게 사용 할 수 있었으면 좋겠습니다. (.properties, .xml 파일 등등)
   			</a></li>
   			
   			</br>
   			<li><a>
   				개발에 필요한 library 를 추가하고 싶은데 메이븐 중앙리파지토리에 존재 하지 않을 경우 이를 egov 넥서스 서버로 3rd part 형태로 업로드 할 수 있는 방법을 제공해 주셨으면 합니다.
		...
		<p>
		이상입니다. Egov 의 무궁한 발전을 기원합니다.^^ 감사합니다.
		</p>  
	</div>
	-->
</div>
